...
<xsl:variable name="transcript" select="document('transcript.xml')/t:transcript"/> 
    ...
    <xsl:template match="apcv:cv" > 
            <profile>
                ...
                <xsl:value-of select="$empRecord"/> <endRecord/>
                <xsl:value-of select="$empRecord/e:empName"/>  <endEmpName/>
                <xsl:value-of select="$empRecord/e:workedAt/e:year"/> <endworkedAt/>
                <xsl:value-of select="$empRecord/e:workedAt"/> <endworkedAt/>
